...
class EGAT_Rxn(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.Aggregate == 'Concat':
            if cfg.AddonAgg is True:
                self.mlp1 = nn.Sequential(nn.Linear(self.hidden_dim*self.num_heads*4 + 400, 256, bias=True),nn.GELU())
            else:
                self.mlp1 = nn.Sequential(nn.Linear(self.hidden_dim*self.num_heads*4 + 200, 256, bias=True),nn.GELU())
        else:
            if cfg.AddonAgg is True:
                self.mlp1 = nn.Sequential(nn.Linear(self.hidden_dim*self.num_heads*2 + 400, 256, bias=True),nn.GELU())
            else:
                self.mlp1 = nn.Sequential(nn.Linear(self.hidden_dim*self.num_heads*2 + 200, 256, bias=True),nn.GELU())# BLOCK3: final MLP layers
        self.mlp2 = nn.Sequential(nn.Linear(256, 128, bias=True),nn.GELU())
        
        self.mlp3 = nn.Linear(128+len(cfg.additionals), 1, bias=True)


    def forward(self, graphR, graphP, Hr,Radd,Padd):
