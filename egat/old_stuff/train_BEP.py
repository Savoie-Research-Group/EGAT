...
@hydra.main(config_path='config', config_name='EGAT_BEP_3MLP')
def main(args):
    
    omegaconf.OmegaConf.set_struct(args, False)
    wandb.init(project='EGAT_BEP')
    ...
    #TRAIN_DATASET = RGD1Dataset(root=root, split='train', class_choice='R1P1', return_RxnE=True)
    ...
    #TEST_DATASET = RGD1Dataset(root=root, split='val', class_choice='R1P1', return_RxnE=True)
    ...
    for epoch in range(start_epoch, args.epoch+args.epoch_const):
        ...
        '''learning one epoch'''
        loss_list = []
        for i, (Rgs, Pgs, targets) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            
            target    = torch.Tensor([float(i[2]) for i in targets]).view(args.batch_size,1).to(device)
            Hr        = torch.Tensor([float(i[3]) for i in targets]).view(args.batch_size,1).to(device)
        ...
        with torch.no_grad():
            ...
            eval_loss = []
            for batch_id, (Rgs, Pgs, targets) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                outputs = predictor(RGgs, PGgs)
                pred = outputs[:, 0].unsqueeze(1) * Hr + outputs[:, 1].unsqueeze(1)
                eval_loss.append(criterion(pred, target).cpu().data.numpy())

        # compute the average
        wandb.log({"learning_rate":lr, "train_loss": train_instance_acc, 'val_loss': np.mean(eval_loss)},step=epoch)
