...
@hydra.main(config_path='config', config_name='EGAT_direct_3MLP')
def main(args):
    
    omegaconf.OmegaConf.set_struct(args, False)
    wandb.init(project='EGAT_direct')
    ...
    TRAIN_DATASET = RGD1Dataset(root=root, split='train', class_choice=None, exclude=exclude)
    ...
    TEST_DATASET = RGD1Dataset(root=root, split='val', class_choice=None, exclude=exclude)
    ...
    for epoch in range(start_epoch, args.epoch+args.epoch_const):
        ...
        for Rgs, Pgs, targets in tqdm(trainDataLoader, total=len(trainDataLoader), smoothing=0.9):
            ...
            target    = torch.Tensor([float(i[-1]) for i in targets]).view(args.batch_size,1).to(device)
            RGgs      = Rgs.to(device)
            PGgs      = Pgs.to(device)
