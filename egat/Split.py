...
def Split(args):
    ...
    if args.datastorage is None:
        ...
        elif args.split_type == 'preset':
            df = pd.read_csv(args.input,index_col=0)
            split_set = df.loc[:,'Split']

            train = split_set[split_set == 'train']
            test = split_set[split_set == 'test']
            val = split_set[split_set == 'val']
        else:
            ...
            # Take the training and validation and run x-fold cross validation. 
                ...
                for fold_idx, (train_idx, val_idx) in enumerate(kf.split(combined_train_val), 1):
                    ...
                    with open(os.path.join(args.data_path,f'train_test_split/fold{fold_idx}/train_idx.json'),'w') as f:
                        json.dump(fold_train_data,f)
                    
                    with open(os.path.join(args.data_path,f'train_test_split/fold{fold_idx}/val_idx.json'),'w') as f:
                        json.dump(fold_val_data,f)

                    with open(os.path.join(args.data_path,f'train_test_split/fold{fold_idx}/test_idx.json'),'w') as f:
                        json.dump(test,f)
            elif args.split_type == 'preset':
                df = pd.read_csv(args.input,index_col=0)
                split_set = df.loc[:,[f'fold{i}Split' for i in len(args.folds)]]
                
                for fold in range(args.folds):
                    train = split_set[split_set == f'train']
                    test = split_set[split_set == f'test']
                    val = split_set[split_set == f'val']
        ...
        conn = sqlite3.connect(os.path.join(args.data_path,args.data_path.split('/')[-1]+'.db'))
        cursor = conn.cursor()
            ...
            for i,names in enumerate([train,val,test]):
                for table in names:
                    if table[0] != 'sqlite_sequence':
                        ...
                        insert_query = f"INSERT INTO {table[0]} (key, value) VALUES (?, ?)"
                        cursor.execute(insert_query, (f'split', sets[i]))
                        #print(f'{table[0]} is executed')
                
            conn.commit()
            conn.close()
                            
                        
                        #cursor.execute(f"ALTER TABLE {[table[0]]} ADD COLUMN split TEXT;")
                    #cursor.execute(f"UPDATE {[table[0]]} SET split={sets[i]};")
                ...
                for fold_idx, (train_idx, val_idx) in enumerate(kf.split(combined_train_val), 1):
                    ...
                    for i,names in enumerate([train,val,test]):
                        for table in names:
                            if table[0] != 'sqlite_sequence':
                                insert_query = f"INSERT INTO {table[0]} (key, value) VALUES (?, ?)"
                                cursor.execute(insert_query, (f'fold{fold_idx}_split', sets[i]))
