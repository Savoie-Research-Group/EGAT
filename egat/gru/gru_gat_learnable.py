...
class GRU(nn.Module):
    ...
    >>> egat = EGATConv(init_node_feats=20,
                          in_edge_feats=12,
                          out_node_feats=15,
                          out_edge_feats=10,
                          num_heads=3)
    ...
    def __init__(self,
        ...
        self._mid_node_feats = middle_node_feats
        self._mid_edge_feats = middle_edge_feats
        
        self._init_node_feats = init_node_feats
        self._in_edge_feats = init_edge_feats
            

        self._scaling = scaling

        self.r_hi = nn.Linear(init_node_feats+middle_node_feats, out_node_feats*num_heads*scaling, bias=True)
        self.r_hj = nn.Linear(init_node_feats+middle_node_feats, out_node_feats*num_heads*scaling, bias=True)
        self.r_eij = nn.Linear(init_edge_feats+middle_edge_feats, out_node_feats*num_heads*scaling, bias=True)

        self.z_hi = nn.Linear(init_node_feats+middle_node_feats, out_node_feats*num_heads*scaling, bias=True)
        self.z_hj = nn.Linear(init_node_feats+middle_node_feats, out_node_feats*num_heads*scaling, bias=True)
        self.z_eij = nn.Linear(init_edge_feats+middle_edge_feats, out_node_feats*num_heads*scaling, bias=True)

        self.htil_hi = nn.Linear(init_node_feats+middle_node_feats, out_node_feats*num_heads*scaling, bias=True)
        self.htil_hj = nn.Linear(init_node_feats+middle_node_feats, out_node_feats*num_heads*scaling, bias=True)
        self.htil_eij = nn.Linear(init_edge_feats+middle_edge_feats, out_node_feats*num_heads*scaling, bias=True)
    ...
    def edge_recurrent_unit(self, edges):
        #extract features
        f = edges.data['f']
        #stack h_i | f_ij | h_j
    ...
    def node_recurrent_unit(self, edges):
        #extract features
        h_src = edges.src['h']
        h_dst = edges.dst['h']
        #stack h_i | f_ij | h_j
