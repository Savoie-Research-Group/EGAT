...
class EGATConvSA(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_node_attention = nn.Linear(out_node_feats, num_heads, bias=False)
    ...
    def reset_parameters(self):
        ...
        init.xavier_normal_(self.fc_node_attention.weight, gain=gain)
    ...
    def edge_attention(self, edges):
        ...
        self.edge_attn = a
        return {'a_edge': a, 'f' : f_out}

    def node_attention(self, nodes):
        #extract features
        h = nodes.data['h']
        nfeats_ = self.fc_nodes(h)
        nfeats_ = nfeats_.view(-1, self._num_heads, self._out_node_feats)

        # apply FC and activation
        nfeats_ = nn.functional.leaky_relu(nfeats_)
        nfeats_ = nfeats_.view(-1, self._num_heads, self._out_edge_feats)
        # apply FC to reduce edge_feats to scalar
        a_node = self.fc_node_attention(nfeats_).sum(-1).unsqueeze(-1)
        self.self_attn = a_node
        
        return {'a_node': a_node, 'h' : nfeats_}
    ...
    def reduce_func(self, nodes):
        ...
        # Extract edge and node attention scores from the mailbox
        a_edge = nodes.mailbox['a_edge']
        a_node = nodes.mailbox['a_node']
        
        # Concatenate edge and node attention scores along the specified dimension
        concatenated_a = th.cat([a_edge, a_node], dim=1)
        
        # Apply softmax to the concatenated attention scores
        alpha = nn.functional.softmax(concatenated_a, dim=1)
        
        # Weighted sum of neighboring node features using the softmax values
        h_neighbor = th.sum(alpha * nodes.mailbox['h'], dim=1)
        
        alpha_node = alpha[:, -a_node.size(1):]  # Extract alpha_node from the concatenated alpha
        # Weighted sum of the same node feature using the softmax values
        h_self = th.sum(alpha_node * nodes.data['h'], dim=1)
        # Combine the two weighted sums
        h = h_neighbor + h_self
        return {'h': h}
    ...
    def forward(self, graph, nfeats, efeats):
        ...
        ##TODO allow node src and dst feats
            ...
            graph.apply_nodes(self.node_attention)
