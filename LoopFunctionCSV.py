...
'''
getelectronegatvity=False,getipot=False,getpolarizability=False,getis=False,
gethbondinfo=False,getspiroinfo=False,getbridgeinfo=False,getabinfo=False,getbondpolarity=False
'''
...
removebondorderinfo=False,removeconjinfo=False,removestereoinfo=False,getRDKitFeatures=False,getRDKITNormfatures=False,datastorage=None):
        ###### READ CSV FILE 
        ...
        atom_hybrid_encode = {Chem.HybridizationType.S: [0,0,0,1], Chem.HybridizationType.SP: [0,0,1,0], Chem.HybridizationType.SP2: [0,1,0,0], Chem.HybridizationType.SP3: [1,0,0,0]}
        ...
        try:
            ...
            if not molecular: 
                if atom_map is True:
                    ...
                    info["Psmiles"] = NPsmiles 
            else:
                if atom_map is True:
                    NRsmiles = deepcopy(Rsmiles)
                    Rsmiles = Chem.MolFromSmiles(explicitflags=False)
            ...
            try:
                ...
                if datastorage is None:
                    ...
                    table_name = data_path.split('/')[-1] + '_' + Rxntype + '_' + Rind
                    ...
                    database_name = data_path.split('/')[-1]
                    collection_name = data_path.split('/')[-1] + '_' + Rxntype + '_' + Rind
            ...
            with open(os.path.join(data_path,'fail.txt'),'a') as ff:
                    ff.write('{}\n'.format(Rind))
                    if not molecular:
                        ff.write(f'The reaction between failed because of {e} \n')
                    else:
                        ff.write(f'The reaction between failed because of {e} \n')
            import traceback
            traceback.print_exc()
